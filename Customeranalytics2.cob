IDENTIFICATION DIVISION.
PROGRAM-ID. EmployeeBonusCalculator.

ENVIRONMENT DIVISION.
INPUT-OUTPUT SECTION.
FILE-CONTROL.
    SELECT EMPLOYEE-FILE ASSIGN TO 'EMPLOYEE.DAT' ORGANIZATION IS LINE SEQUENTIAL.
    SELECT BONUS-REPORT ASSIGN TO 'BONUS_REPORT.TXT' ORGANIZATION IS LINE SEQUENTIAL.

DATA DIVISION.
FILE SECTION.
FD EMPLOYEE-FILE.
01 EMPLOYEE-RECORD.
   05 EMPLOYEE-ID PIC 9(5).
   05 EMPLOYEE-NAME PIC X(25).
   05 SALARY-PART-1 PIC 9(5).
   05 SALARY-DECIMAL-POINT PIC X.
   05 SALARY-PART-2 PIC 9(2).
   05 PERFORMANCE-RATING PIC 9.

FD BONUS-REPORT.
01 BONUS-RECORD PIC A(80).

WORKING-STORAGE SECTION.
01 END-OF-FILE PIC X VALUE 'N'.
   88 EOF VALUE 'Y'.
   88 NOT-EOF VALUE 'N'.
01 BONUS-RATE PIC V99 VALUE 0.
01 BONUS-AMOUNT PIC 9(7)V99 VALUE 0.
01 REPORT-LINE PIC A(80).
01 WS-ANNUAL-SALARY PIC 9(7)V99.
01 WS-SALARY-COMBINED PIC 9(7).

PROCEDURE DIVISION.
000-MAIN.
    OPEN INPUT EMPLOYEE-FILE
         OUTPUT BONUS-REPORT
    PERFORM UNTIL EOF
        READ EMPLOYEE-FILE INTO EMPLOYEE-RECORD
            AT END
                SET EOF TO TRUE
            NOT AT END
                DISPLAY "Reading Employee ID: " EMPLOYEE-ID
                DISPLAY "Employee Name: " EMPLOYEE-NAME
                MOVE SALARY-PART-1 TO WS-SALARY-COMBINED (1:5)
                MOVE SALARY-PART-2 TO WS-SALARY-COMBINED (6:2)
                MOVE WS-SALARY-COMBINED TO WS-ANNUAL-SALARY
                DISPLAY "Annual Salary: " WS-ANNUAL-SALARY
                DISPLAY "Performance Rating: " PERFORMANCE-RATING
                EVALUATE PERFORMANCE-RATING
                    WHEN 1 THRU 2
                        MOVE 0 TO BONUS-RATE
                    WHEN 3
                        MOVE 0.10 TO BONUS-RATE
                    WHEN 4 THRU 5
                        MOVE 0.20 TO BONUS-RATE
                    WHEN OTHER
                        MOVE 0 TO BONUS-RATE
                END-EVALUATE
                COMPUTE BONUS-AMOUNT = WS-ANNUAL-SALARY * BONUS-RATE
                MOVE SPACES TO REPORT-LINE
                STRING EMPLOYEE-NAME " - Rating: " PERFORMANCE-RATING
                       " - Bonus: $" BONUS-AMOUNT DELIMITED BY SIZE INTO REPORT-LINE
                WRITE BONUS-RECORD FROM REPORT-LINE
        END-READ
    END-PERFORM

    CLOSE EMPLOYEE-FILE
          BONUS-REPORT
    STOP RUN.
